		...
		if (this.shouldCustomize(e.getGui())) {
			...
			if (!FancyMenu.config.getOrDefault("hidebranding", true)) {
				BrandingControl.forEachLine(true, true, (brdline, brd) ->
						GuiComponent.drawString(matrix, font, brd, 2, e.getGui().height - ( 10 + brdline * (font.lineHeight + 1)), 16777215)
				);
			}

			if (!FancyMenu.config.getOrDefault("hideforgenotifications", false)) {
				ForgeHooksClient.renderMainMenu((TitleScreen) e.getGui(), matrix, Minecraft.getInstance().font, e.getGui().width, e.getGui().height, 255);
				BrandingControl.forEachAboveCopyrightLine((brdline, brd) ->
						GuiComponent.drawString(matrix, font, brd, e.getGui().width - font.width(brd) - 1, e.getGui().height - (11 + (brdline + 1) * ( font.lineHeight + 1)), 16777215)
				);
			}
